...
const DIAGNOSTIC_SOURCE = "GovernSpec";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const outputChannel = vscode.window.createOutputChannel("GovernSpec");

  context.subscriptions.push(diagnostics, outputChannel);
  context.subscriptions.push(
    vscode.commands.registerCommand("governspec.validateCurrentFile", async () => {
      await validateCurrentFile(diagnostics, outputChannel);
    }),
    vscode.commands.registerCommand("governspec.inspectCurrentFile", async () => {
      await inspectCurrentFile(outputChannel);
    }),
    vscode.commands.registerCommand("governspec.compileCurrentFile", async () => {
      await compileCurrentFile(outputChannel);
    }),
    vscode.commands.registerCommand("governspec.testOutput", async () => {
      await testOutput(outputChannel);
    }),
    vscode.commands.registerCommand("governspec.showMcpSetupSnippet", async () => {
...
async function validateCurrentFile(
  ...
  if (validationResult.ok === false) {
    vscode.window.showWarningMessage("GovernSpec validation reported issues.");
    return;
  }
  vscode.window.showInformationMessage("GovernSpec validation completed.");
}
...
async function compileCurrentFile(outputChannel: vscode.OutputChannel): Promise<void> {
  ...
  const target = await vscode.window.showQuickPick(COMPILE_TARGETS, {
    placeHolder: "Select a GovernSpec compile target",
  });
  if (!target) {
    return;
  }
  ...
  vscode.window.showInformationMessage(`GovernSpec compile completed: ${target}`);
}
...
async function showMcpSetupSnippet(): Promise<void> {
  const mcpCommand = getConfiguredCommand("mcpCommand", "governspec-mcp");
  const snippet = {
    mcpServers: {
      governspec: {
        command: mcpCommand,
...
function getActiveDocument(): vscode.TextDocument | undefined {
  ...
  if (!editor) {
    void vscode.window.showWarningMessage("Open a GovernSpec file first.");
    return undefined;
  }
...
function getConfiguredCommand(
  key: "command" | "mcpCommand",
  defaultValue: string
): string {
  return vscode.workspace
    .getConfiguration("governspec")
...
async function runJsonCommand(
  ...
  if (!payload) {
    throw new Error("GovernSpec command returned no output.");
  }
...
async function runCommand(
  ...
  const governSpecCommand = getConfiguredCommand("command", "governspec");
  return new Promise((resolve, reject) => {
    const child = spawn(governSpecCommand, args, {
    ...
    child.on("close", (code) => {
      ...
      reject(new Error(stderr || stdout || `GovernSpec exited with code ${code}.`));
    });
  });
}
