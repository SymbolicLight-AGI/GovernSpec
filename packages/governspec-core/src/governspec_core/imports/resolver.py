...
from governspec_core.common.errors import GovernSpecValidationError
from governspec_core.common.utils import dedupe_preserve_order, deep_merge_dict, resolve_path
from governspec_core.spec.models import GovernDocument, GovernPack, GovernSpec
from governspec_core.spec.parser import load_document

_MERGEABLE_FIELDS = {"permissions", "constraints", "human_gates", "tests", "quality", "evidence"}


def resolve_imports(spec: GovernSpec) -> GovernSpec:
    resolved = resolve_document_imports(spec)
    if not isinstance(resolved, GovernSpec):
        raise GovernSpecValidationError("Resolved document is not a GovernSpec.")
    return resolved


def resolve_document_imports(document: GovernDocument) -> GovernDocument:
    resolved_payload, resolved_paths = _resolve_document(document, stack=[])
    resolved: GovernDocument
    if isinstance(document, GovernSpec):
        resolved = GovernSpec.model_validate(resolved_payload)
    else:
        resolved = GovernPack.model_validate(resolved_payload)
...
def _resolve_document(
    document: GovernDocument,
    *,
    stack: list[Path],
) -> tuple[dict[str, Any], list[Path]]:
    source_path = document._source_path
    if source_path is None:
        raise GovernSpecValidationError("Loaded document is missing source path metadata.")
    ...
    if isinstance(document, GovernPack):
        pack_payload = {
