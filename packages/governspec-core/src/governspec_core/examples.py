"""Packaged examples for GovernSpec."""
...
from governspec_core.common.utils import write_text
...
def list_examples() -> list[ExampleFile]:
    resource_dir = resources.files("governspec_core.resources.examples")
    examples: list[ExampleFile] = []
    for relative_name, file in _walk_examples(resource_dir):
        if file.is_file() and file.name.endswith((".govern.yaml", ".output.md", ".output.json")):
...
def copy_example(name: str, destination: str | Path) -> Path:
    resource_name = name
    resource_file = resources.files("governspec_core.resources.examples").joinpath(resource_name)
