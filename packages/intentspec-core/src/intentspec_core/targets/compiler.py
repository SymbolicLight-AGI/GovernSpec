"""Compile IntentSpec into target artifacts."""
...
from intentspec_core.common.utils import dump_json, render_markdown_list
...
def compile_target(spec: IntentSpec, target: str) -> CompiledArtifact:
...
def _compile_prompt(iir: NormalizedIntent) -> str:
    ...
    parts = [
        f"# {iir.metadata.get('title') or iir.metadata.get('name', 'IntentSpec Task')}",
        "",
        "## Goal",
        iir.normalized_goal,
        "",
...
def _compile_openai_structured(iir: NormalizedIntent) -> str:
    ...
    payload = {
        "type": "json_schema",
        "json_schema": {
            "name": iir.metadata.get("name", "intent_output"),
...
def _compile_cursor_rules_bundle(iir: NormalizedIntent) -> dict[str, str]:
    ...
    return {".cursor/rules/intentspec.mdc": rendered}


def _compile_antigravity_rules_bundle(iir: NormalizedIntent) -> dict[str, str]:
    return {
        ".agents/rules/intentspec.md": _render_rule_document(
            iir,
            title="IntentSpec Antigravity Rules",
        )
    }
...
def _compile_skill_bundle(iir: NormalizedIntent) -> dict[str, str]:
    skill_md = "\n".join(
        [
            "---",
            f"name: {iir.metadata.get('name', 'intent-skill')}",
