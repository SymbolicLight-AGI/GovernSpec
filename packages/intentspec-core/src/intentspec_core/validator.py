"""Semantic validation for IntentSpec v0.1."""

from __future__ import annotations

from dataclasses import asdict, dataclass, field

from intentspec_core.common.utils import resolve_path
...
def validate_spec(spec: IntentSpec) -> ValidationReport:
    return _validate_spec(spec)


def validate_document(document: IntentDocument) -> ValidationReport:
    if isinstance(document, IntentSpec):
        return _validate_spec(document)
    return _validate_pack(document)


def _validate_spec(spec: IntentSpec) -> ValidationReport:
    errors: list[str] = []
    warnings: list[str] = []
...
def _validate_pack(pack: IntentPack) -> ValidationReport:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if not any(
        ...
        warnings.append(
            "IntentPack does not define reusable governance fields yet."
        )

    return ValidationReport(ok=not errors, errors=errors, warnings=warnings)


def _has_confirmation_gate(document: IntentSpec | IntentPack, keywords: tuple[str, ...]) -> bool:
...
def _uses_external_network(spec: IntentSpec) -> bool:
    return _uses_external_network_from_permissions(spec.permissions)


def _uses_high_risk_tool(spec: IntentSpec) -> bool:
...
def _has_evidence_section(spec: IntentSpec) -> bool:
...
def _has_confidential_inputs(spec: IntentSpec) -> bool:
    return any(item.privacy.lower() == "confidential" for item in spec.inputs)


def _mentions_privacy_constraints(spec: IntentSpec) -> bool:
