"""Import AGENTS.md / CLAUDE.md instruction documents into IntentSpec drafts.

Supports two parsing modes:

1. **IntentSpec-generated** documents — recognized by the fixed section
...
def import_instruction_markdown(text: str) -> dict[str, Any]:
    """Parse an AGENTS.md or CLAUDE.md document and return a draft IntentSpec dict."""
    text = strip_frontmatter(text)
    sections = split_sections(text)

    if _is_intentspec_generated(sections):
        return _parse_intentspec_generated(sections)
    return _parse_handwritten(text, sections)


def _is_intentspec_generated(sections: dict[str, str]) -> bool:
    return len(_INTENTSPEC_GENERATED_SECTIONS & set(sections.keys())) >= 4


def _parse_intentspec_generated(sections: dict[str, str]) -> dict[str, Any]:
