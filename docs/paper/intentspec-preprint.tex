...
\date{Preprint draft, April 2026}
...
\texttt{intent.yaml} file. IntentSpec compiles that contract into the native
...
The starting point for IntentSpec is the rapid expansion of LLM-assisted software
...
IntentSpec follows that hybrid direction. It does not attempt to improve the model
...
actions require human confirmation, and how outputs should be checked. IntentSpec
...
IntentSpec differs from a prompt catalog or an agent framework. Prompt patterns
describe reusable prompting strategies. Agent frameworks orchestrate model calls,
memory, tools, and actions. IntentSpec is a compilation and validation layer for the
...
JSON documents (Wright et al., 2022). IntentSpec uses this ecosystem when compiling
structured-output targets and when checking JSON outputs offline.

IntentSpec is also adjacent to policy-as-code. Policy-as-code systems encode
...
adopted and maintained in open-source repositories (Foalem et al., 2026). IntentSpec
borrows the same engineering instinct but applies it to AI-agent task artifacts. The
important distinction is that IntentSpec is not a runtime authorization engine. It
is a local contract compiler and acceptance checker for artifacts that may feed
different agent tools.

\section{Design Goals}

IntentSpec is organized around five design goals.
...
single source of governance intent. IntentSpec therefore emits artifacts that
...
permissions or human gates. IntentSpec records these losses as capability notes.
...
\begin{quote}
\small\texttt{python} \path{benchmark/paper_icse2027/scripts/run_all.py}
\end{quote}
...
The important observation is not that every target succeeds. Rather, IntentSpec
...
\subsection{Assisted Annotation Agreement}

The expanded benchmark also records a pilot consistency check over 29 output
samples. Two Codex-assisted annotation passes labeled \texttt{expected\_ok},
\texttt{targeted\_assertion}, \texttt{failure\_scope}, and \texttt{output\_format}
according to a written annotation protocol. The labels are used as an assisted
annotation sanity check, not as independent human-human agreement. As shown in
Table~\ref{tab:agreement}, \texttt{expected\_ok} and \texttt{output\_format}
agreement were perfect; \texttt{targeted\_assertion} agreement was 82.76\%
(Cohen's kappa 0.7917); and \texttt{failure\_scope} agreement was 93.1\%
(Cohen's kappa 0.8612). The disagreements mainly occur when a malformed JSON output
can be interpreted as either a schema failure or a missing-path failure.

\begin{table}[htbp]
\caption{Assisted annotation agreement.}
\label{tab:agreement}
\centering
\begin{tabular}{lrrr}
\toprule
Field & Items & Agreement & Kappa \\
\midrule
\texttt{expected\_ok} & 29 & 100.0\% & 1.0 \\
\texttt{targeted\_assertion} & 29 & 82.76\% & 0.7917 \\
\texttt{failure\_scope} & 29 & 93.1\% & 0.8612 \\
\texttt{output\_format} & 29 & 100.0\% & 1.0 \\
\bottomrule
\end{tabular}
\end{table}
...
payloads. IntentSpec imports each artifact into a draft contract and compares the
...
existing artifacts, IntentSpec supports incremental adoption. The fidelity score
