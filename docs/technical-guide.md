...
IntentSpec 是一个本地优先的 AI 任务契约工具。它解决的问题不是“再做一个 agent
...
> IntentSpec 让团队用一份任务契约定义目标、权限、约束、人工确认门、输出格式和验收测试，然后编译到多个 AI 工具，并在产出后做离线检查。
...
IntentSpec 可以把这些要求放进 `intent.yaml`，再编译成目标平台需要的文件或 JSON
payload。

### 使用 Cursor、Codex、Claude Code 的工程团队

如果团队同时使用多种 AI coding 工具，IntentSpec 可以作为上游规则源。你不需要为每个工具手写一份规则，而是维护一份 contract：

- 给 Codex 或其他读取 repository instructions 的工具，编译成 `AGENTS.md`。
- 给 Claude Code，编译成 `CLAUDE.md`。
- 给 Cursor，编译成 `.cursor/rules/intentspec.mdc`。
...
如果你关心 AI agent 的权限边界、人工确认、隐私和输出质量，IntentSpec 可以把这些内容变成显式字段，而不是埋在 prompt 里：
...
IntentSpec 也适合做 AI agent 工具链研究。它提供：
...
IntentSpec 做的是 artifact-level governance：编译规则、迁移规则、检查输出。真正的运行时权限控制仍然需要由操作系统、agent runtime、MCP server、CI/CD 或企业安全系统承担。
...
schema 和 CI 脚本里。IntentSpec 把它们统一到一个 `intent.yaml`：
...
IntentSpec 会解析 imports，并按保守规则合并：权限取更严格结果，列表合并去重，导入的测试断言不会被本地同名测试静默覆盖。

### 离线验收报告

IntentSpec 可以在 agent 产出之后做 deterministic checks：
...
IntentSpec v0.1 需要 Python 3.11+。
...
IntentSpec 的核心流程不需要 API key，不需要联网，也不会调用真实 LLM。
...
这会生成默认 `intent.yaml`。你可以修改 `goal`、`permissions`、`constraints`、
`output` 和 `tests`。
...
```bash
intent compile intent.yaml --target gemini-structured --out task.gemini-structured.json
```

### 第五步：让 agent 工作

在目标工具里正常使用 agent。IntentSpec 不接管 agent runtime，它只是生成目标工具能读取的 artifact。

例如：

- Codex 读取 `AGENTS.md`。
- Claude Code 读取 `CLAUDE.md`。
- Cursor 读取 `.cursor/rules/intentspec.mdc`。
...
```bash
intent test report_json.intent.yaml --output output.json
```
...
```bash
intent draft "Review this codebase without modifying files and produce a concise report." --out draft.intent.yaml
```

中文也可以：

```bash
intent draft "帮我写客户会议简报，不要泄露隐私，涉及敏感数据先问我" --out draft.intent.yaml
```
...
导入结果是 draft，不是严格等价证明。自然语言 artifact 的导入一定存在启发式判断，所以导入后应该运行 `intent validate` 和 `intent inspect`，再由人确认。
...
```bash
intent import AGENTS.md --out migrated.intent.yaml
intent inspect migrated.intent.yaml
intent compile migrated.intent.yaml --target cursor-rules --out .
```
...
- 对每个重要 `intent.yaml` 运行 `intent validate`。
- 对团队实际使用的 targets 运行 `intent compile`，防止 schema 或 target capability 变化导致编译失败。
- 对示例输出或 golden outputs 运行 `intent test`。
- 对导入后的 draft 运行 `intent inspect --format json`，保存 review artifact。

## 9. 与 MCP 的关系

IntentSpec 提供一个薄 MCP server：
...
MCP 集成的用途是让 MCP-capable clients 直接读取和使用 IntentSpec 的 validation、inspection、compilation 和 testing 能力。它不是通用文件读取服务，路径访问应限制在项目工作区内。
...
IntentSpec 默认本地运行，不要求真实 LLM、不要求 API key、不要求联网。这让它适合进入 CI、研究 benchmark 和企业内网环境。

### Zero-intrusion

IntentSpec 不要求下游 agent runtime 改造。它通过现有 artifact channel 集成：markdown instructions、rules 文件、structured output JSON 和 MCP surface。

### Source-of-truth

`intent.yaml` 是唯一源文件。下游文件是编译结果，不应该手动长期维护。如果需要改规则，先改 contract，再重新 compile。
...
编译出 `AGENTS.md` 或 Cursor Rules 不等于 agent 一定会遵守。IntentSpec 帮你把规则放到正确位置，并在输出后检查一部分可验证约束，但不能控制模型内部行为。
...
`openai-structured` 和 `gemini-structured` 适合 JSON schema 输出，但不能表达所有自然语言约束、human gates 或 evidence policy。IntentSpec 会尽量在编译时暴露这些 target limitation。

### Regex 需要谨慎

正则断言适合简单模式，不建议写复杂、不可维护的表达式。IntentSpec 对正则执行做了超时保护，但规则作者仍应保持模式简单。

## 13. 最佳实践

- 每个重要 agent 任务都维护一份 `intent.yaml`。
- 把跨任务规则抽成 `IntentPack`。
- 把生成的 `AGENTS.md`、`CLAUDE.md`、Cursor Rules 视为 build artifact。
- 在 PR 中 review `intent.yaml`，不要只 review 编译后的 markdown。
- 对关键输出样本维护 golden output，并用 `intent test` 放进 CI。
- 对 structured output 任务优先写明确 `json_schema`。
- 对高风险权限使用 `human_gates`。
- 对导入结果一定运行 `intent inspect` 并人工确认。
...
| `packages/intentspec-cli` | `intent` CLI |
| `packages/intentspec-mcp` | MCP server |
