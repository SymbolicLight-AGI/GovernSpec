...
def test_prompt_target_includes_goal_and_constraints() -> None:
    prompt = compile_target(
        resolve_imports(load_spec(EXAMPLES / "customer_brief.govern.yaml")),
        "prompt",
    )
...
def test_openai_structured_target_is_valid_json() -> None:
    rendered = compile_target(load_spec(EXAMPLES / "report_json.govern.yaml"), "openai-structured")
...
def test_agents_md_target_is_text() -> None:
    rendered = compile_target(load_spec(EXAMPLES / "code_review.govern.yaml"), "agents-md")
...
def test_claude_md_target_is_text() -> None:
    rendered = compile_target(load_spec(EXAMPLES / "code_review.govern.yaml"), "claude-md")
    assert rendered.content is not None
...
def test_cursor_rules_target_returns_bundle() -> None:
    rendered = compile_target(load_spec(EXAMPLES / "code_review.govern.yaml"), "cursor-rules")
    assert rendered.kind == "bundle"
    assert ".cursor/rules/governspec.mdc" in rendered.files
    assert "alwaysApply: true" in rendered.files[".cursor/rules/governspec.mdc"]


def test_antigravity_rules_target_returns_bundle() -> None:
    rendered = compile_target(
        load_spec(EXAMPLES / "code_review.govern.yaml"),
        "antigravity-rules",
    )
    assert rendered.kind == "bundle"
    assert ".agents/rules/governspec.md" in rendered.files
    assert "# GovernSpec Antigravity Rules" in rendered.files[".agents/rules/governspec.md"]


def test_skill_target_returns_bundle() -> None:
    rendered = compile_target(load_spec(EXAMPLES / "code_review.govern.yaml"), "skill")
    assert rendered.kind == "bundle"
...
def test_gemini_structured_target_is_valid_json() -> None:
    rendered = compile_target(load_spec(EXAMPLES / "report_json.govern.yaml"), "gemini-structured")
    payload = json.loads(rendered.content or "")
...
def test_gemini_structured_rejects_unsupported_schema_keywords(tmp_path: Path) -> None:
    spec_path = tmp_path / "unsupported.govern.yaml"
...
""".strip(),
    ...
    with pytest.raises(GovernSpecCompileError):
        compile_target(load_spec(spec_path), "gemini-structured")


def test_unsupported_target_raises_compile_error() -> None:
    with pytest.raises(GovernSpecCompileError):
        compile_target(load_spec(EXAMPLES / "customer_brief.govern.yaml"), "unknown-target")
