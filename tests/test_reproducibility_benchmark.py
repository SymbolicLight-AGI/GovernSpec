...
BENCHMARK = ROOT / "benchmark" / "reproducibility"
SCRIPTS = BENCHMARK / "scripts"


def _load_helpers():
    module_path = SCRIPTS / "benchmark_helpers.py"
    spec = importlib.util.spec_from_file_location("benchmark_helpers_for_tests", module_path)
...
def test_reproducibility_dataset_is_consistent() -> None:
...
def test_run_all_generates_reproducibility_outputs(tmp_path: Path) -> None:
    out_dir = tmp_path / "results"
    result = subprocess.run(
        [
            sys.executable,
